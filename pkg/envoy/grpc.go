...
func createLDSServer(path string) *LDSServer {
	...
	os.Remove(path)
	var err error
	lds_server.lis, err = net.ListenUnix("unix", &net.UnixAddr{path, "unix"})
	if err != nil {
	...
	lds_server.listener_proto = &envoy_api.Listener{
		...
		FilterChains: []*envoy_api.FilterChain{{
			Filters: []*envoy_api.Filter{{
				...
				Config: &structpb.Struct{map[string]*structpb.Value{
					"stat_prefix": {&structpb.Value_StringValue{"proxy"}},
					"http_filters": {&structpb.Value_ListValue{&structpb.ListValue{[]*structpb.Value{
						{&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
							"name": {&structpb.Value_StringValue{"cilium_l7"}},
							"config": {&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
								"deprecated_v1": {&structpb.Value_BoolValue{true}},
							}}}},
							"deprecated_v1": {&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
								"type": {&structpb.Value_StringValue{"decoder"}},
							}}}},
						}}}},
						{&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
							"name": {&structpb.Value_StringValue{"router"}},
							"config": {&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
								"deprecated_v1": {&structpb.Value_BoolValue{true}},
							}}}},
							"deprecated_v1": {&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
					...
					"rds": {&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
						"config_source": {&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
							"api_config_source": {&structpb.Value_StructValue{&structpb.Struct{map[string]*structpb.Value{
								"api_type":     {&structpb.Value_NumberValue{float64(envoy_api.ApiConfigSource_GRPC)}},
								"cluster_name": {&structpb.Value_StringValue{"rdsCluster"}},
							}}}},
						}}}},
						// "route_config_name": {&structpb.Value_StringValue{"route_config_name"}},
					}}}},
				}},
...
func (s *LDSServer) addListener(name string, port uint32, l7rules []AuxRule) {
	...
	listener.listenerConf.Address.GetSocketAddress().PortSpecifier = &envoy_api.SocketAddress_PortValue{port}
	listener.listenerConf.FilterChains[0].Filters[0].Config.Fields["rds"].GetStructValue().Fields["route_config_name"] = &structpb.Value{&structpb.Value_StringValue{name}}
...
func createRDSServer(path string, lds *LDSServer) *RDSServer {
	...
	os.Remove(path)
	var err error
	rds_server.lis, err = net.ListenUnix("unix", &net.UnixAddr{path, "unix"})
	if err != nil {
		log.Fatalf("failed to listen: %v", err)
	}
	...
	rds_server.allow_action = envoy_api.Route_Route{&envoy_api.RouteAction{
		ClusterSpecifier: &envoy_api.RouteAction_Cluster{"cluster1"},
	}}
...
func (p *RDSServer) translatePolicyRule(h api.PortRuleHTTP) *envoy_api.Route {
	...
	is_regex := wrappers.BoolValue{true}
	headers := make([]*envoy_api.HeaderMatcher, 0, cnt)
	if h.Path != "" {
		headers = append(headers, &envoy_api.HeaderMatcher{":path", h.Path, &is_regex})
	}
	if h.Method != "" {
		headers = append(headers, &envoy_api.HeaderMatcher{":method", h.Method, &is_regex})
	}

	if h.Host != "" {
		headers = append(headers, &envoy_api.HeaderMatcher{":authority", h.Host, &is_regex})
	}
	...
	return &envoy_api.Route{
		Match: &envoy_api.RouteMatch{
			PathSpecifier: &envoy_api.RouteMatch_Prefix{"/"},
			Headers:       headers,
		},
...
func (s *RDSServer) StreamRoutes(rds envoy_api.RouteDiscoveryService_StreamRoutesServer) error {
	...
	for {
		...
		for _, name := range names {
			...
			l.HandleVersion(&ctx, version, func() {
				s.pushRoutes(rds, l)
			})
		}
	}
...
func (s *LDSServer) StreamListeners(lds envoy_api.ListenerDiscoveryService_StreamListenersServer) error {
	...
	for {
		...
		s.HandleVersion(&ctx, version, func() {
			log.Printf("StreamListeners: Pushing changed listeners.")
			s.pushListeners(lds)
		})
	}
	ctx.Stop()
...
func createBootstrap(filePath string, name, cluster, version string, ldsName, ldsSock, rdsName, rdsSock string, envoyClusterName string) {
	bs := &envoy_api.Bootstrap{
		Node: &envoy_api.Node{name, cluster, nil, nil, version},
			...
			Clusters: []*envoy_api.Cluster{
				{
					...
					ConnectTimeout: &duration.Duration{1, 0},
