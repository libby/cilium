...
func (ctrl *StreamControl) startHandler(ctx *StreamControlCtx, handler func()) {
	ctrl.handled = true
...
// 'handler()' is called to send the current version if it is later than 'version'
// Starts a handler gorouting on demand tracked by 'ctx'.
func (ctrl *StreamControl) HandleVersion(ctx *StreamControlCtx, version uint64, handler func()) {
	ctrl.cond.L.Lock()
